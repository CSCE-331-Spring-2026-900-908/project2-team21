...
SELECT order_id, employee_id, order_timestamp, total_amount FROM orders ORDER BY total_amount DESC LIMIT 1; --Finds the single largest transaction by revenue that day
...
SELECT TO_CHAR(order_timestamp, 'YYYY-MM') AS sale_month, COUNT(order_id) AS total_orders, SUM(total_amount) AS monthly_revenue FROM orders GROUP BY sale_month ORDER BY sale_month; --Shows total orders and revenue by month in the past year